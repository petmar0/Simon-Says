/*
 Modified 05/03/2015
 Pete Marchetto
 This version of the code introduces a delay adjustment made by putting in a variable voltage of 0-5 VDC from a 10k potentiometer to the analog input at A1.
 This version is also specifically designed to work with the MagicGlove rehabilitation devices.
 This code also allows for setting the number of fingers to be worked with by setting a four-position voltage divider of 0-5 VDC into the analog input at A2.
 For further information, contact me at petmar@gmail.com
...
void playMoves(void)
{
  for (byte currentMove = 0 ; currentMove < gameRound ; currentMove++) 
  {
    ...
    delay(map(analogRead(A1),0,1023,150,1000)); // Delay ranges based on the analog input from the potentiometer.
  }
}
...
{
  int bNum=4;
  bNum=map(analogRead(A2),0,1023,0,4);
  byte newButton = random(0, bNum); //min (included), max (excluded)
...
byte wait_for_button(void)
{
  ...
  while ( (millis() - startTime) < t) // Loop until too much time has passed
  {
